...
CACHE = {}; CACHE_TIME = {}; TRANS = {}
PCACHE = {"data":None,"time":0}; DCACHE = {"data":None,"time":0}
...
def get_detailed_prices():
    ...
    try:
        ...
        rates=fx.get("rates",{})
        gd=requests.get("https://api.gold-api.com/price/XAU",timeout=5).json()
        sd=requests.get("https://api.gold-api.com/price/XAG",timeout=5).json()
        gold_oz=gd.get("price",4286) if isinstance(gd,dict) else 4286
        silver_oz=sd.get("price",31.5) if isinstance(sd,dict) else 31.5
        ...
        OZ=31.1035; gg=gold_oz/OZ; gk=gg*1000; sg=silver_oz/OZ; sk=sg*1000
        ...
        currencies=[
            ...
            {"name":"السعودي 🇸🇦","from":f"1$ = {rates.get('SAR',3.75):.2f} ر.س","to":f"1 ر.س = ${1/rates.get('SAR',3.75):.4f}"},
            {"name":"اليورو 🇪🇺","from":f"1$ = {rates.get('EUR',0.92):.3f} €","to":f"1 € = ${1/rates.get('EUR',0.92):.3f}"},
            {"name":"الكويتي 🇰🇼","from":f"1$ = {rates.get('KWD',0.307):.3f} د.ك","to":f"1 د.ك = ${1/rates.get('KWD',0.307):.2f}"},
            {"name":"المغربي 🇲🇦","from":f"1$ = {rates.get('MAD',9.8):.2f} د.م","to":f"1 د.م = ${1/rates.get('MAD',9.8):.4f}"},
            {"name":"الجزائري 🇩🇿","from":f"1$ = {rates.get('DZD',134):.1f} د.ج","to":f"1 د.ج = ${1/rates.get('DZD',134):.5f}"},
            {"name":"المصري 🇪🇬","from":f"1$ = {rates.get('EGP',50.5):.2f} جنيه","to":f"1 جنيه = ${1/rates.get('EGP',50.5):.4f}"},
        ]
        data={
            "currencies":currencies,
            "gold":{"usd_g":gg,"usd_oz":gold_oz,"usd_kg":gk,"syp_g":to_syp(gg),"syp_oz":to_syp(gold_oz),"syp_kg":to_syp(gk),"try_g":to_try(gg),"try_oz":to_try(gold_oz)},
            "silver":{"usd_g":sg,"usd_oz":silver_oz,"usd_kg":sk,"syp_g":to_syp(sg),"syp_oz":to_syp(silver_oz),"try_g":to_try(sg)},
            "crypto":[
                {"n":"Bitcoin ₿","s":"BTC","p":cr.get("bitcoin",{}).get("usd",67200),"c":cr.get("bitcoin",{}).get("usd_24h_change",0)},
                {"n":"Ethereum ♦","s":"ETH","p":cr.get("ethereum",{}).get("usd",3800),"c":cr.get("ethereum",{}).get("usd_24h_change",0)},
                {"n":"Solana ◎","s":"SOL","p":cr.get("solana",{}).get("usd",145),"c":cr.get("solana",{}).get("usd_24h_change",0)},
                {"n":"Dash","s":"DASH","p":cr.get("dash",{}).get("usd",28),"c":cr.get("dash",{}).get("usd_24h_change",0)},
                {"n":"BNB","s":"BNB","p":cr.get("binancecoin",{}).get("usd",620),"c":cr.get("binancecoin",{}).get("usd_24h_change",0)},
                {"n":"USDT 💵","s":"USDT","p":cr.get("tether",{}).get("usd",1),"c":0},
            ],
            "updated":datetime.now().strftime("%H:%M:%S")
        }
        DCACHE["data"]=data; DCACHE["time"]=now; return data
    except:
        return {"currencies":[{"name":"السورية 🇸🇾","from":"1$ = 15,250 ل.س","to":"1 ل.س = $0.000065"},{"name":"التركية 🇹🇷","from":"1$ = 34.20 TL","to":"1 TL = $0.0292"}],"gold":{"usd_g":137.7,"usd_oz":4286,"usd_kg":137700,"syp_g":2100000,"syp_oz":65361500,"syp_kg":2100000000,"try_g":4710,"try_oz":146500},"silver":{"usd_g":1.01,"usd_oz":31.5,"usd_kg":1012,"syp_g":15400,"syp_oz":480375,"try_g":34.6},"crypto":[{"n":"Bitcoin ₿","s":"BTC","p":67200,"c":1.2},{"n":"Ethereum","s":"ETH","p":3800,"c":-0.5}],"updated":datetime.now().strftime("%H:%M:%S")}
...
def trans_ar(t):
    ...
    try:
        ...
        tr="".join([s[0] for s in r[0]]); TRANS[t]=tr; return tr
    except: return t
...
def fetch_one(args):
    ...
    try:
        ...
        for e in f.entries[:4]:
            ...
            res.append({"id":urllib.parse.quote(e.link),"title":title,"link":e.link,"time":getattr(e,'published','')[:16],"source":src["name"],"image":img})
        return res
    except: return []
    ...
    if q:
        try:
            ...
            for e in feed.entries[:12]:
                img=get_img(e) or get_og(e.link) or DEFAULT_IMG
                res.append({"id":urllib.parse.quote(e.link),"title":trans_ar(e.title),"link":e.link,"time":getattr(e,'published','')[:16],"source":"بحث 🔍","image":img})
            CACHE[key]=res; CACHE_TIME[key]=now; return res
        except: return []
    all_news=[]
    sources=EXTRA.get(cat,[])[:5]
    if sources:
    ...
    try:
        if FEEDS.get(cat) and FEEDS.get(cat)!="PRICES":
            ...
            for e in fg.entries[:2]:
                img=get_img(e) or get_og(e.link) or DEFAULT_IMG
                all_news.append({"id":urllib.parse.quote(e.link),"title":trans_ar(e.title),"link":e.link,"time":getattr(e,'published','')[:16],"source":"Google","image":img})
    except: pass
...
def admin():
    if request.method=='POST':
        try:
            ...
            save_data({"syp_black":syp}); PCACHE["time"]=0; DCACHE["time"]=0
        except: pass
        return redirect('/admin')
    data=load_data()
    return f'<html dir="rtl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>لوحة تحكم شامي</title><style>body{{font-family:Tajawal;padding:20px;background:#f5f5f5}}.box{{background:#fff;padding:20px;border-radius:12px;max-width:500px;margin:auto}}input{{width:100%;padding:12px;border:1px solid #ddd;border-radius:8px;margin:10px 0}}button{{background:#1b5e20;color:#fff;border:0;padding:12px;border-radius:8px;width:100%;font-weight:800}}</style></head><body><div class="box"><h2>🔧 لوحة تحكم شامي</h2><form method="post"><label>سعر الدولار سوق سوداء</label><input name="syp" value="{data.get("syp_black",15250)}"><button>حفظ ✅</button></form><br><a href="/">رجوع للموقع</a></div></body></html>'

@app.route('/article')
def article_page():
    url=request.args.get('url','')
    if not url: return redirect('/')
    title=trans_ar(request.args.get('title','خبر من شامي'))
    img=request.args.get('img',DEFAULT_IMG)
    source=request.args.get('source','شامي')
    try:
        r=requests.get(url, headers={'User-Agent':'Mozilla/5.0'}, timeout=5)
        txt=re.sub(r'<[^>]+>',' ', r.text); txt=re.sub(r'\s+',' ', txt)[:3000]
    except: txt="افتح المصدر الأصلي للمقال كامل"
    wa=urllib.parse.quote(title+' '+url)
    return f'''<!doctype html><html dir="rtl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{title}</title><link href="https://fonts.googleapis.com/css2?family=Tajawal:wght@700&display=swap" rel="stylesheet"><style>*{{font-family:Tajawal}}body{{margin:0;background:#f2f3f5}}.hdr{{background:#1b5e20;color:#fff;padding:12px;position:sticky;top:0}}.container{{max-width:750px;margin:auto;padding:12px}}.card{{background:#fff;border-radius:16px;overflow:hidden}} .card img{{width:100%;height:280px;object-fit:cover}} .body{{padding:16px}} .btns{{display:flex;gap:10px}} .btn{{flex:1;padding:12px;border-radius:12px;text-align:center;text-decoration:none;font-weight:800}} .btn-b{{background:#111;color:#fff}} .btn-w{{background:#25D366;color:#fff}}</style></head><body><div class="hdr"><a href="/" style="color:#fff;text-decoration:none">⬅ رجوع</a> شامي</div><div class="container"><div class="card"><img src="{img}"><div class="body"><small>{source}</small><h1>{title}</h1><p style="line-height:1.8">{txt}</p><div class="btns"><a href="{url}" target="_blank" class="btn btn-b">المصدر الأصلي</a><a href="https://wa.me/?text={wa}" class="btn btn-w">واتساب</a></div></div></div></div></body></html>'''
...
def home_render(cat,news,q=None):
    ...
    try:
        ...
        ticker=" 🔴 ".join([n["title"] for n in urgent])
    ...
    cards=""
    for n in news:
        detail=f"/article?url={urllib.parse.quote(n['link'])}&title={urllib.parse.quote(n['title'])}&img={urllib.parse.quote(n['image'])}&source={urllib.parse.quote(n['source'])}"
        cards+=f'<article class="card"><div class="imgw"><img src="{n["image"]}" loading="lazy" onerror="this.src=\'{DEFAULT_IMG}\'"><span class="badge">{n["source"]}</span></div><div class="info"><small>{n["time"]}</small><h2>{n["title"]}</h2><div class="btns"><a href="{detail}" class="btn-r">📖 داخل شامي</a><a href="https://wa.me/?text={urllib.parse.quote(n["title"]+" "+n["link"])}" target="_blank" class="btn-w">واتساب</a></div></div></article>'
    return f'''<!doctype html><html dir="rtl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>شامي</title><link rel="manifest" href="/manifest.json"><link href="https://fonts.googleapis.com/css2?family=Tajawal:wght@700;800&display=swap" rel="stylesheet"><style>*{{font-family:Tajawal;box-sizing:border-box}}body{{margin:0;background:#f2f3f5;transition:.3s}}:root{{--bg:#f2f3f5;--card:#fff;--text:#111}}[data-theme=dark]{{--bg:#121212;--card:#1e1e1e;--text:#eee}}body{{background:var(--bg);color:var(--text)}}.top{{background:linear-gradient(90deg,#0d3b1f,#1b5e20);color:#fff;padding:12px;display:flex;justify-content:space-between;align-items:center;position:sticky;top:0;z-index:30}}.ticker{{background:#d32f2f;color:#fff;display:flex;align-items:center;height:44px;overflow:hidden}}.label{{background:#fff;color:#d32f2f;font-weight:900;padding:6px 14px;border-radius:6px;margin:0 10px;white-space:nowrap}}.prices{{background:#0e0e0e;color:#ddd;display:flex;gap:16px;overflow:auto;padding:9px 12px;font-size:13px;white-space:nowrap}}.tabs{{display:flex;gap:10px;overflow:auto;padding:12px;background:var(--card);position:sticky;top:56px;z-index:20}}.tab{{padding:10px 18px;background:#eceff1;border-radius:24px;text-decoration:none;color:#333;font-weight:700;font-size:13px;white-space:nowrap}}.tab.active{{background:#1b5e20;color:#fff}}.container{{max-width:760px;margin:auto;padding:12px}}.card{{background:var(--card);border-radius:18px;overflow:hidden;margin:14px 0;box-shadow:0 6px 20px rgba(0,0,0,.07)}}.imgw{{position:relative}}.imgw img{{width:100%;height:230px;object-fit:cover}}.badge{{position:absolute;top:12px;right:12px;background:rgba(0,0,0,.7);color:#fff;padding:5px 10px;border-radius:20px;font-size:11px}}.info{{padding:14px}}.info h2{{margin:6px 0 12px;font-size:16px;line-height:1.5}}.btns{{display:flex;gap:10px}}.btn-r,.btn-w{{flex:1;text-align:center;padding:12px;border-radius:12px;text-decoration:none;font-weight:800}}.btn-r{{background:#111;color:#fff}}.btn-w{{background:#25D366;color:#fff}}.search{{background:var(--card);padding:10px;position:sticky;top:110px;z-index:19}}.search form{{max-width:760px;margin:auto;display:flex;gap:8px}}.search input{{flex:1;padding:12px 16px;border:1px solid #ddd;border-radius:24px}}.search button{{background:#1b5e20;color:#fff;border:0;padding:12px 20px;border-radius:24px;font-weight:800}}</style><script>let th=localStorage.getItem('shami_theme')||'light'; if(th=='dark') document.documentElement.setAttribute('data-theme','dark'); function toggle(){{let c=document.documentElement.getAttribute('data-theme'); let n=c=='dark'?'light':'dark'; if(n=='light') document.documentElement.removeAttribute('data-theme'); else document.documentElement.setAttribute('data-theme','dark'); localStorage.setItem('shami_theme',n)}} if('serviceWorker' in navigator) navigator.serviceWorker.register('/sw.js'); function notif(){{if('Notification' in window) Notification.requestPermission().then(p=>{{if(p=='granted') new Notification('شامي 🔥',{{body:'رح توصلك الأخبار العاجلة!'}})}})}}</script></head><body><div class="top"><div style="display:flex;align-items:center;gap:8px"><div style="width:36px;height:36px;background:#fff;color:#1b5e20;border-radius:8px;display:flex;align-items:center;justify-content:center;font-weight:900">ش</div><div><b>شامي 🔥</b><br><small style="font-size:11px;opacity:.8">أخبار + أسعار حية</small></div></div><div style="display:flex;gap:6px"><button onclick="toggle()" style="background:rgba(255,255,255,.2);border:0;color:#fff;padding:8px 12px;border-radius:20px">🌙/☀️</button><button onclick="notif()" style="background:rgba(255,255,255,.2);border:0;color:#fff;padding:8px 12px;border-radius:20px">🔔</button><a href="/admin" style="background:rgba(255,255,255,.2);color:#fff;padding:8px 12px;border-radius:20px;text-decoration:none">⚙️</a></div></div><div class="ticker"><span class="label">عاجل 🔴</span><marquee scrollamount="7" direction="right">{ticker}</marquee></div><div class="prices"><span>₿ {prices["btc"]}</span><span>ETH {prices["eth"]}</span><span>🪙 {prices["gold"]}</span><span>💵 دمشق {prices["syp_black"]} ل.س</span><span style="margin-right:auto"><a href="/?cat=%D8%A3%D8%B3%D8%B9%D8%A7%D8%B1%20%F0%9F%92%B1" style="color:#25D366;text-decoration:none;font-weight:800">كل الأسعار 💱</a></span></div><div class="search"><form action="/search"><input name="q" value="{q or ''}" placeholder="🔍 ابحث..."><button>بحث</button></form></div><div class="tabs">{tabs}</div><div class="container">{cards}</div><div style="text-align:center;padding:20px;color:#888;font-size:12px">شامي © 2026 - PWA + ليلي + مفضلة + قراءة داخلية + إشعارات + لوحة تحكم<br><button onclick="alert('من كروم: ⋮ ثم تثبيت التطبيق / Add to Home screen')" style="margin-top:10px;background:#1b5e20;color:#fff;border:0;padding:10px 18px;border-radius:20px;font-weight:800">📲 ثبّت شامي كتطبيق</button></div></body></html>'''
