...
app = Flask(__name__)
app.secret_key = "shami_reader_fixed"
...
# === قراءة داخل التطبيق بدون beautifulsoup ===
@app.route('/read')
def read_article():
    ...
    try:
        ...
        article_text = "\n".join(clean_paras) if clean_paras else "انقر على فتح المصدر لقراءة الخبر كاملاً"
    except:
        article_text = "المحتوى محمي - اضغط فتح المصدر"
    ...
    <style>body{{margin:0;font-family:Tahoma;background:#f5f5f5}}.h{{background:#b71c1c;color:#fff;padding:12px;display:flex;gap:10px;align-items:center;position:sticky;top:0;z-index:10}}.h a{{color:#fff;text-decoration:none;background:rgba(255,255,255,.2);padding:6px 14px;border-radius:20px}}.c{{max-width:700px;margin:auto;background:#fff}}.c img{{width:100%;max-height:360px;object-fit:cover}}.body{{padding:18px}}.body h1{{font-size:21px;line-height:1.5}}.body p{{font-size:17px;line-height:1.9;margin:14px 0;color:#222}}.acts{{display:flex;gap:10px;padding:16px;position:sticky;bottom:0;background:#fff;border-top:1px solid #eee}}.btn{{flex:1;padding:12px;text-align:center;border-radius:10px;text-decoration:none;font-weight:bold}}.src{{background:#111;color:#fff}}.wa{{background:#25D366;color:#fff}}</style></head>
    <body><div class="h"><a href="javascript:history.back()">← رجوع</a><b>قراءة داخلية</b></div>
    <div class="c"><img src="{img}" onerror="this.style.display='none'"><div class="body"><h1>{title}</h1><p style="color:#888;font-size:12px">{urllib.parse.unquote(url)[:70]}</p><hr>
    {"".join([f'<p>{p}</p>' for p in article_text.split(chr(10)) if p.strip()])}
    </div><div class="acts"><a href="{url}" target="_blank" class="btn src">فتح المصدر ↗</a><a href="https://wa.me/?text={urllib.parse.quote(title+' '+url)}" target="_blank" class="btn wa">واتساب</a></div></div></body></html>"""
...
def home():
    ...
    return f"""<!doctype html><html dir="rtl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>شامي نيوز</title><link rel="manifest" href="/manifest.json"><style>body{{margin:0;font-family:Tahoma;background:#f0f2f5}}.h{{background:#b71c1c;color:#fff;padding:12px;display:flex;justify-content:space-between;position:sticky;top:0;z-index:10}}.tabs{{display:flex;gap:8px;overflow:auto;padding:10px;background:#fff;position:sticky;top:56px}}.tab{{padding:8px 14px;background:#eee;border-radius:20px;text-decoration:none;color:#333;white-space:nowrap}}.tab.active{{background:#d32f2f;color:#fff}}.c{{max-width:700px;margin:auto;padding:10px}}.card{{background:#fff;border-radius:12px;overflow:hidden;margin:12px 0}}.card img{{width:100%;height:190px;object-fit:cover}}.body{{padding:12px}}.btns{{display:flex;gap:8px}}.r,.w{{flex:1;text-align:center;padding:10px;border-radius:8px;color:#fff;font-weight:bold;text-decoration:none}}.r{{background:#111}}.w{{background:#25D366}} #upd{{font-size:11px;opacity:.8}}</style></head>
    <body><div class="h"><div><h1 style="margin:0;font-size:17px">🔥 شامي</h1><div id="upd">يتحدث كل دقيقة</div></div><a href="/admin" style="color:#fff;text-decoration:none">⚙️</a></div><div class="tabs">{tabs}</div><div class="c" id="newsContainer">{cards}</div>
    <script>
