...
app = Flask(__name__)
app.secret_key = "shami_sport_fast"
...
# نظام الكاش السريع - كل المصادر محفوظة
CACHE = {}
def fetch_all_news():
    while True:
        for cat in FEEDS:
            try:
                ...
                print(f"تم تحديث {cat}: {len(all_news)} خبر")
            except: pass
        time.sleep(180) # كل 3 دقايق

# شغل التحديث بالخلفية
...
def get_news(cat):
    ...
    if not cached:
        return cf + [{"title":f"جاري تحميل {cat}... ثواني","link":"#","time":datetime.now().strftime("%H:%M"),"source":"شامي","image":"https://images.unsplash.com/photo-1461896836934-ffe607ba8211?w=600","wa":"#","is_custom":False}]
    return cf + cached
...
def read_article():
    ...
    try:
        ...
        clean_paras=[re.sub(r'<[^>]+>', '', p).strip() for p in paragraphs[:20] if len(re.sub(r'<[^>]+>', '', p).strip())>50]
...
def home():
    ...
    <body><div class="h"><div><h1 style="margin:0;font-size:17px">🔥 شامي - سريع ⚡ كل المصادر</h1><div style="font-size:11px">8 مصادر رياضة شغالة بالخلفية</div></div><a href="/admin" style="color:#fff;text-decoration:none">⚙️</a></div><div class="tabs">{tabs}</div><div class="c" id="newsContainer">{cards}</div>
    <script>
