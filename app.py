...
app = Flask(__name__)
app.secret_key = "shami_final_2026"
...
if os.path.exists(CUSTOM_FILE):
    try:
        CUSTOM_NEWS = json.load(open(CUSTOM_FILE, encoding="utf-8"))
    except: pass
    ...
    try:
        ...
        for e in feed.entries[:20]:
            gn.append({
                ...
                "source": "Google",
                "image": get_image(e),
                "wa": f"https://wa.me/?text={urllib.parse.quote(e.title)}",
                "is_custom": False
            })
        if not gn: raise Exception("empty")
        return cf + gn
    except:
        # لو فشل، رجع أخبارك الخاصة + خبر وهمي مشان ما يبين فاضي
        if cf: return cf
        return [{"title": f"جاري تحديث أخبار {cat}... حاول بعد دقيقة","link":"#","time":datetime.now().strftime("%H:%M"),"source":"شامي","image":"https://images.unsplash.com/photo-1504711434969-e33886168f5c?w=600","wa":"#","is_custom":False}]
...
def admin():
    ...
    if request.method == 'POST':
        if request.form.get('password'):
            ...
            else: return "<h2 style='text-align:center;color:red'>الباسوورد غلط</h2><a href='/admin'>رجوع</a>"
...
def home():
    ...
    for i,n in enumerate(news):
        ad = '<div class="card ad">مساحة إعلانية</div>' if i==5 else ""
        badge = '<div class="custom-badge">خاص 🔥</div>' if n.get('is_custom') else ''
        cards+=f'<div class="card">{badge}<img src="{n["image"]}" loading="lazy"><div class="body"><span>{n["source"]} | {n["time"]}</span><h2><a href="{n["link"]}" target="_blank">{n["title"]}</a></h2><div class="btns"><a href="{n["link"]}" target="_blank" class="r">اقرأ</a><a href="{n["wa"]}" target="_blank" class="w">واتساب</a></div></div></div>{ad}'
    ...
    <style>*{{box-sizing:border-box}}:root{{--bg:#f0f2f5;--card:#fff;--text:#111}}body.dark{{--bg:#121212;--card:#1e1e1e;--text:#eee}}body{{margin:0;font-family:Tahoma;background:var(--bg);color:var(--text)}}.h{{background:#b71c1c;color:#fff;padding:16px;display:flex;justify-content:space-between;align-items:center;position:sticky;top:0;z-index:10}}.tabs{{display:flex;gap:8px;overflow:auto;padding:10px;background:var(--card);position:sticky;top:68px}}.tab{{padding:8px 14px;background:#eee;border-radius:20px;text-decoration:none;color:#333;white-space:nowrap}}.tab.active{{background:#d32f2f;color:#fff}}.c{{max-width:700px;margin:auto;padding:10px}}.card{{background:var(--card);border-radius:12px;overflow:hidden;margin:12px 0;position:relative}}.card img{{width:100%;height:200px;object-fit:cover}}.body{{padding:12px}}.btns{{display:flex;gap:8px}}.r,.w{{flex:1;text-align:center;padding:10px;border-radius:8px;color:#fff;font-weight:bold}}.r{{background:#111}}.w{{background:#25D366}}.live{{width:8px;height:8px;background:#0f0;border-radius:50%;display:inline-block;animation:pulse 1s infinite}}@keyframes pulse{{0%{{opacity:1}}50%{{opacity:.3}}100%{{opacity:1}}}}#upd{{font-size:11px;opacity:.7}}</style></head>
    <body><div class="h"><div><h1 style="margin:0">🔥 شامي <span class="live"></span></h1><div id="upd">آخر تحديث: الآن - يتحدث كل دقيقة</div></div><div><button onclick="toggleDark()" style="background:rgba(255,255,255,.2);border:0;color:#fff;padding:6px 10px;border-radius:20px">🌙</button></div></div><div class="tabs">{tabs}</div><div class="c" id="newsContainer">{cards}</div>
    <script>
    ...
    async function refreshNews(){{
        try{{
            ...
            news.forEach((n,i)=>{{
                ...
                const badge=n.is_custom?'<div style="background:#b71c1c;color:#fff;padding:4px 8px;font-size:12px;position:absolute;top:0;right:0">خاص 🔥</div>':'';
