...
# ===== نفس مصادرك من أول كود بالضبط - ما حذفنا شي =====
FEEDS = {
...
EXTRA={
    "الكل": POLITICS[:1] + SPORTS[:1],
    "عاجل 🔴": POLITICS[:3],
    "رياضة ⚽": SPORTS, # فقط رياضة - هاد الإصلاح 1
    "سياسة 🏛️": POLITICS, # فقط سياسة - هاد الإصلاح 1
...
def get_real_prices():
    ...
    try:
        ...
        g=requests.get("https://api.gold-api.com/price/XAU",headers=HEADERS,timeout=5).json()
...
def get_detailed_prices():
    ...
    try:
        ...
        rates=fx.get("rates",{}); gd=requests.get("https://api.gold-api.com/price/XAU",headers=HEADERS,timeout=5).json(); sd=requests.get("https://api.gold-api.com/price/XAG",headers=HEADERS,timeout=5).json()
        gold_oz=gd.get("price",4286) if isinstance(gd,dict) else 4286; silver_oz=sd.get("price",31.5) if isinstance(sd,dict) else 31.5
        cr=requests.get("https://api.coingecko.com/api/v3/simple/price?ids=bitcoin,ethereum,solana,binancecoin,tether&vs_currencies=usd&include_24hr_change=true",headers=HEADERS,timeout=5).json()
...
def trans_ar(t):
    ...
    try:
        ...
        r=requests.get(url,headers=HEADERS,timeout=3).json(); tr="".join([s[0] for s in r[0]]); TRANS[t]=tr; return tr
    except: return t
...
def get_news(cat,q=None):
    ...
    if q:
        try:
            resp=requests.get(f"https://news.google.com/rss/search?q={urllib.parse.quote(q)}&hl=ar&gl=SA&ceid=SA:ar",headers=HEADERS,timeout=6)
            feed=feedparser.parse(resp.content); res=[]
    ...
    # إصلاح 1: كل قسم من مصادرو فقط - بدون خلط
    if cat in ["رياضة ⚽", "سياسة 🏛️", "اقتصاد 💰", "أسعار 💱"]:
        if sources:
        ...
        try:
            if FEEDS.get(cat) and FEEDS.get(cat)!="PRICES":
                resp=requests.get(FEEDS.get(cat),headers=HEADERS,timeout=6)
                fg=feedparser.parse(resp.content)
    ...
    if not all_news and key in CACHE:
        return CACHE[key]
