...
app = Flask(__name__)
app.secret_key = "shami_global_final"

FEEDS = {
    "الكل": "https://news.google.com/rss/search?q=سوريا+العالم&hl=ar&gl=SA&ceid=SA:ar",
    ...
    "اقتصاد 💰": "https://news.google.com/rss/search?q=اقتصاد+عالمي&hl=ar&gl=SA&ceid=SA:ar",
    "سياسة 🏛️": "https://news.google.com/rss/search?q=سياسة+سوريا+عالم&hl=ar&gl=SA&ceid=SA:ar",
...
EXTRA_FEEDS = {
    "الكل": ["https://sana.sy/feed/", "https://www.enabbaladi.net/feed/", "https://www.bbc.com/arabic/index.xml", "https://www.skynewsarabia.com/web/rss"],
    ...
    "فن 🎨": ["https://www.snacksyrian.com/feed/", "https://www.skynewsarabia.com/web/rss"]
}
...
def get_news(cat):
    ...
    try:
        ...
        for e in feed.entries[:8]:
            all_news.append({"title":e.title,"link":e.link,"time":getattr(e,'published','')[:16] or datetime.now().strftime("%H:%M"),"source":"سوريا + عالم 🌍","image":get_image(e),"wa":f"https://wa.me/?text={urllib.parse.quote(e.title)}","is_custom":False})
    except: pass
    for url in EXTRA_FEEDS.get(cat, EXTRA_FEEDS["الكل"])[:2]:
        try:
            feed2 = feedparser.parse(url)
            for e in feed2.entries[:4]:
                src = "سوري 🇸🇾" if "sana" in url or "enabbaladi" in url or "snack" in url else "عربي عالمي 🌍"
                all_news.append({"title":e.title,"link":e.link,"time":getattr(e,'published','')[:16],"source":src,"image":get_image(e),"wa":f"https://wa.me/?text={urllib.parse.quote(e.title)}","is_custom":False})
        except: continue
