...
<body>
<div class="top-bar">
<span><i class="fa-solid fa-dollar-sign"></i> دولار: {{ prices.usd_syp }}/{{ prices.usd_syp_sell }} ل.س</span>
...
<div><i class="fa-solid fa-magnifying-glass" style="margin-left:12px"></i><i class="fa-solid fa-bars"></i></div>
</div>
{% if urgents %}<div class="urgent-bar"><i class="fa-solid fa-bell"></i> عاجل: {{ urgents[0].title }}</div>{% endif %}
...
def admin():
    ...
    return '<div dir="rtl" style="padding:20px;font-family:Tahoma"><h2>تحكم شامي</h2><form method="post"><input name="title" placeholder="العنوان" style="width:100%;padding:10px"><br><br><textarea name="content" placeholder="المحتوى" style="width:100%;height:80px"></textarea><br><br><input name="image" placeholder="رابط الصورة" style="width:100%;padding:10px"><br><br><select name="category"><option>عاجل</option><option>سياسة</option><option>اقتصاد</option><option>رياضة</option></select><br><br><button>نشر</button></form></div>'
