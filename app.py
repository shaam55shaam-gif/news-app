...
def get_news(cat):
    ...
    for src in EXTRA_FEEDS.get(cat, [])[:2]:
        try:
            url = src["url"]; name = src["name"]
    ...
    CACHE[cat] = all_news[:20]
    CACHE_TIME[cat] = now
    return cf + all_news[:20] if (cf or all_news) else [{"title":f"لا يوجد أخبار في {cat}","link":"#","time":datetime.now().strftime("%H:%M"),"source":"شامي","image":DEFAULT_IMAGES.get(cat),"wa":"#","is_custom":False}]
